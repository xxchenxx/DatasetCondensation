...
from utils import get_loops, get_dataset, get_network, get_eval_pool, evaluate_synset, get_daparam, match_loss, get_time, TensorDataset, epoch, DiffAugment, ParamDiffAug, train_synset_with_previous, evaluate_synset_with_previous
...
def main():
    ...
    for interval_idx in range(args.num_intervals):
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                for model_eval in model_eval_pool:
                    ...
                    for it_eval in range(args.num_eval):
                        ...
                        _, acc_train, acc_test = evaluate_synset_with_previous(it_eval, net_eval, image_syn_eval, label_syn_eval, previous_images_train, previous_labels_train, testloader, args)
                ...
                ''' visualize and save '''
                save_name = os.path.join(args.save_path, 'vis_%s_%s_%s_%dipc_exp%d_iter%d.png'%(args.method, args.dataset, args.model, args.ipc, interval_idx, it))
            ...
            net = train_synset_with_previous(net, previous_images_train, previous_labels_train, args)
            net.train()
            ...
            if 'BN' not in args.model: # for ConvNet
                ...
                loss += torch.sum((torch.mean(output_real.reshape(num_classes, args.batch_real, -1), dim=1) - torch.mean(output_syn.reshape(num_classes, (interval_idx + 1) * args.ipc, -1), dim=1))**2)
